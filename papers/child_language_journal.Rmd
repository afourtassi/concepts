...
\textbf{Age} & Mean Age (months) & SD & Median & Children & Transcripts & Utterances & Words \\
\hline

\textbf{\textless3} & 26.5 & 5.9 & 27.5 & 660 & 4,939 & 1.996 M & 8.262 M\\

\textbf{\textless4} & 30.1 & 8.2 & 30 & 843 & 6,750 & 2.657 M & 11.270 M\\

\textbf{\textless5} & 33.1 & 11 & 31.3 & 971 & 7,889 & 3.093 M & 13.393 M\\

\textbf{\textless6} & 35 & 13 & 32.1 & 1,046 & 8,654 & 3.221 M & 13.991 M\\
...
```{r dev_2, fig.env = "figure", fig.pos = "!htbp", fig.align = "center", fig.width=3, fig.height=3, fig.cap = "\\label{fig:dev_2} The Area Under the ROC Curve (AUC) scores for each cue (across all categories) using speech heard by children within a particular age range. A value of 0.5 represents pure chance, and a value of 1 represents perfect performance."}

dev <- feather::read_feather("../saved/aggregate_2.feather")

dev <- dev %>% filter(grepl("-", yr, fixed=TRUE)) %>% rename(
    value = AUC,
    mode = cue,
    yr = AgeRange) 

mean <- dev %>% dplyr::group_by(cue, AgeRange) %>% dplyr::summarise(AUC = mean(AUC))
mean$cue2 <- ifelse(mean$cue == "cooccurrence", "Pragmatic", ifelse(mean$cue == "w2v", "Co-occurrence", "Affordance"))

ggplot(data=mean, aes(x=AgeRange, y=AUC, group=cue2, col=cue2)) +
  geom_point(position=position_dodge()) +
    geom_line() +
    ylim(0, 1) + 
  geom_hline(yintercept=0.5, linetype = "dashed") +
...
dev2 <- dev %>% filter(!grepl("-", yr, fixed=TRUE)) %>% rename(
    yr = Age,
    value = AUC,
    mode = cue)

mean2 <- dev2 %>% dplyr::group_by(cue, Age) %>% dplyr::summarise(AUC = mean(AUC))
mean2$cue2 <- ifelse(mean2$cue == "cooccurrence", "Pragmatic", ifelse(mean2$cue == "w2v", "Co-occurrence", "Affordance"))

ggplot(data=mean2, aes(x=Age, y=AUC, group=cue2, col=cue2)) +
  geom_point(position=position_dodge()) +
    geom_line() +
    ylim(0, 1) + 
  geom_hline(yintercept=0.5, linetype = "dashed") +
