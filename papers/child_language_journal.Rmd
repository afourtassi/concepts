...
```{r dev_2, include =F, fig.env = "figure", fig.pos = "!htbp", fig.align = "center", fig.width=3, fig.height=3, fig.cap = "\\label{fig:dev_2} The Area Under the ROC Curve (AUC) scores for each cue (across all categories) using speech heard by children within a particular age range. A value of 0.5 represents pure chance, and a value of 1 represents perfect performance."}

dev <- feather::read_feather("../saved/aggregate_2.feather")

dev <- dev %>% dplyr::filter(grepl("-", yr, fixed=TRUE)) %>% rename(
    AUC = value,
    cue = mode,
    AgeRange = yr) 

mean <- dev %>% dplyr::group_by(cue, AgeRange) %>% dplyr::summarise(AUC = mean(AUC))
mean$cue2 <- plyr::mapvalues(mean$cue, from = c("w2v", "cooccurrence", "verbs"), to = c("Co-occurrence", "Pragmatic", "Affordance"))
