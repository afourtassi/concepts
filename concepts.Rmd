...
get_childes_utterances <- function(){
  ...
  #write.csv(adult_utterances, "pydata/adult_utteraces_5.csv")
  adult_utterances <- dplyr::filter(adult_utterances, target_child_age < 48)
  #write.csv(adult_utterances, "pydata/adult_utteraces_4.csv")
  adult_utterances <- dplyr::filter(adult_utterances, target_child_age < 36)
  #write.csv(adult_utterances, "pydata/adult_utteraces_3.csv")
...
```{r}
write.csv(explicit_cosSim, "pydata/test.csv")
```
...
for (age in 1:length(txt_files)){
  ...
  w2v_cohesion <- auc_measure(w2v_clust_measure, cue_words) %>%
    ...
    melt() %>% 
    cbind(yr=age+2)
  cohesion <- rbind(cohesion, explicit_cohesion, w2v_cohesion)
}
