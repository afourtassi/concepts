...
Retrieve number of individual children and transcripts used in analysis
```{r}
adult_utterances_6 <- read_csv("pydata/adult_utterances_6.csv")
meta_child <- count(adult_utterances_6, target_child_id)
meta_transcript <- count(adult_utterances_6, transcript_id)
...
<!-- Can you edit the script to be able to run it all from command line? -->
...
get_w2v_cosSim <- function(filename, instances, age){
  ...
  w2v_filename <- paste("rdata/rds/", toString(age + 2), "_vectors_w2v.rds", sep="")
...
get_explicit_cosSim <- function(filename, age, cue_type){
  ...
  df_filename <- paste("rdata/rds/", toString(age + 2), "_vectors_explicit.rds", sep="")
...
for (age in 1:length(txt_files)){
  ...
  #cooccurrence_cosSim <- get_explicit_cosSim(cooccurrence_files[age], age)
  ...
  plot(density_plot(explicit_clust_measure) + ggtitle(paste("Density of Verb Categorization for Age < ", toString(age + 2))))
  plot(density_plot(w2v_clust_measure) + ggtitle(paste("Density of w2v Categorization for Age < ", toString(age + 2)))) 
  plot(density_plot(cooccurrence_clust_measure) + ggtitle(paste("Density of All Co-Occurrence Categorization for Age < ", toString(age + 2)))) 
  
  w2v_clust <- get_clust(w2v_cosSim)
  w2v_clust_file <- paste("rdata/rds/", toString(age + 2), "_clust_w2v.rds", sep="")
  saveRDS(w2v_clust, w2v_clust_file)
  #w2v_clust <- readRDS(w2v_clust_file)

  explicit_clust <- get_clust(explicit_cosSim)
  explicit_clust_file <- paste("rdata/rds/", toString(age + 2), "_clust_explicit.rds", sep="")
  ...
  cooccurrence_clust_file <- paste("rdata/rds/", toString(age + 2), "_clust_cooccurrence.rds", sep="")
  saveRDS(cooccurrence_clust, cooccurrence_clust_file)
  #cooccurrence_clust <- readRDS(cooccurrence_clust_file)

  fviz_cluster(list(data = w2v_cosSim, cluster = w2v_clust), labelsize = 8, main = paste("w2v Cluster for Age < ", toString(age + 2))) 
  fviz_cluster(list(data = explicit_cosSim, cluster = explicit_clust), labelsize = 8, main = paste("Verb Cluster for Age < ", toString(age + 2)))
  fviz_cluster(list(data = cooccurrence_cosSim, cluster = cooccurrence_clust), labelsize = 8, main = paste("Co-Occurrence Cluster for Age < ", toString(age + 2))) 
  ...
  geom_bar(stat="identity", position=position_dodge()) + ylim(0, 1) + ggtitle(paste("AUC for Age < ", toString(age + 2))) + scale_fill_manual("legend", values = c("darkcyan", "darkslateblue", "darkred")))
}
